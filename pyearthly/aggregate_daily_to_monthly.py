...
def aggregate_daily_to_monthly(source_file_path, destination_path, variable, year, data_type, aggregation_method='sum', filename_pattern=None, date_pattern_in_filename='%Y_%m', custom_dimension='time', verbose=False, output_filename_pattern="era5-{data_type}-{year}.nc"):
    """
    Combines daily netCDF files into a single netCDF file with layers corresponding to each day in the source files for a given month.
    ...
    if filename_pattern is None:
        filename_pattern = f"era5-{data_type}-{year}_*.nc"
    ...
    output_filename = output_filename_pattern.format(data_type=data_type, year=year)
